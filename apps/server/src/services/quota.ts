...
async function acquireLock(tx: Tx, key: string): Promise<void> {
  await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${key})::bigint)`);
}

export async function assertVaultQuota(tx: Tx, userId: string, limit: number): Promise<void> {
...
export async function getEffectiveOwnerQuota(tx: Tx, userId: string): Promise<number> {
  const [row] = await tx
    .select({
...
export async function getEffectiveVaultItemQuota(tx: Tx, vaultId: string): Promise<number> {
  const [row] = await tx
    .select({
