...
export function parse(raw: string): ImportResult {
  ...
  const otpAuthIdx = headers.indexOf('otpauth');

  if (passwordIdx === -1 || usernameIdx === -1) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const title = titleIdx !== -1 && cols[titleIdx] ? cols[titleIdx] : (url ?? '');
    ...
    if (otpAuthIdx !== -1 && cols[otpAuthIdx]) {
      const totp = parseTotpUri(cols[otpAuthIdx]);
      if (totp) {
