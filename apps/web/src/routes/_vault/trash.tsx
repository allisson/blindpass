...
function TrashItemRow({
  ...
  return (
    <div
      ...
      className="px-3 py-3 outline-none transition-colors duration-150 hover:bg-accent/40 focus-visible:bg-accent/60 focus-visible:ring-2 focus-visible:ring-ring/40 focus-visible:ring-inset border-b border-border/60 last:border-b-0"
    >
      <div role="cell" className="min-w-0 mb-1.5">
      ...
      <div role="cell" className="min-w-0 mb-2">
        <p className="text-xs text-muted-foreground truncate" title={item.deletedAt}>
          <span className={vaultMissing ? 'italic text-muted-foreground/60' : ''}>
            {vaultMissing ? '(deleted vault)' : vaultName}
          </span>
          <span className="text-muted-foreground/40 mx-1.5">·</span>
          <span className="font-mono tabular-nums tracking-tight">{iso}</span>
          {relative && (
            <span className="font-sans tracking-normal text-muted-foreground/50 ml-1.5">
              · {relative}
            </span>
          )}
        </p>
      ...
      <div role="cell" className="flex items-center gap-1">
        <Button
          variant="ghost"
          size="sm"
          className="flex-1 h-8 text-xs gap-1.5 text-foreground/80 hover:text-primary hover:bg-primary/10"
        ...
        <Button
          variant="ghost"
          size="sm"
          className="h-8 px-2.5 text-xs gap-1.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 shrink-0"
        ...
        <header className="mb-6 flex flex-col gap-3">
          ...
          {hasItems && (
            <Button
              variant="outline"
              size="sm"
              className="self-start text-destructive hover:text-destructive hover:bg-destructive/10 border-destructive/30"
        ...
        {isLoading && (
          ...
          >
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="px-3 py-3 border-b border-border/60 last:border-b-0">
                <div className="space-y-1.5 min-w-0 mb-1.5">
                  <Skeleton className="h-3 w-2/3 rounded" />
                  <Skeleton className="h-2.5 w-1/2 rounded" />
                </div>
                <Skeleton className="h-2.5 w-3/4 rounded mb-2" />
