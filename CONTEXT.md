...
**BiometricCredential**:
The server-side record `{id, userId, credentialId, label, createdAt, lastSeenAt}` that tracks a WebAuthn credential enrolled for **Biometric unlock** on a specific device. Created (`POST /auth/biometric-credentials`) when the client enrolls biometric unlock; the server-assigned `id` (UUID) is stored in the local **BiometricEnrollment** as `serverCredentialId`. Hard-deleted on revocation (`DELETE /auth/biometric-credentials/:id`). The device checks its credential's status (`GET /auth/biometric-credentials/:id`) in parallel with key fetching on every biometric unlock attempt — a 404 means revoked, causing the device to wipe `bp:biometric-unlock` and surface a `credential_revoked` ceremony error before any biometric gesture is requested. A device that is offline can still unlock until it next contacts the server. Distinct from **BiometricEnrollment**: the enrollment holds the wrapped **MasterKey** on the device; the credential is the server's authority record.
_Avoid_: biometric enrollment (server-side), credential record.
...
**Biometric unlock**:
The alternative to **UnlockWithPassword** that recovers **MasterKey** from a local **BiometricEnrollment** by deriving a **BUK** via WebAuthn PRF and decrypting `encryptedMasterKey`. Replaces only the vault-unlock step; the server `bp_session` cookie and TOTP login are unchanged. Available only when a **BiometricEnrollment** exists for `getLastUsername()` on this device and the platform supports the `prf` extension. On two triggers the client checks whether its local enrollment's `credentialId` still appears (non-tombstoned) in `GET /user/biometric-credentials`: (1) before the WebAuthn ceremony in `useBiometricUnlock`, so a revoked credential never prompts biometric; (2) on vault mount in `_vault/route.tsx`, as a fire-and-forget check that silently wipes the local enrollment if it was tombstoned while the device held an active session.
